...
drop policy if exists "level_progress_select_own" on public.level_progress;
create policy "level_progress_select_own" on public.level_progress
  for select using (auth.uid() = user_id);

drop policy if exists "level_progress_write_own" on public.level_progress;
create policy "level_progress_write_own" on public.level_progress
  for insert with check (auth.uid() = user_id);

drop policy if exists "level_progress_update_own" on public.level_progress;
create policy "level_progress_update_own" on public.level_progress
  for update using (auth.uid() = user_id);

drop policy if exists "saved_programs_select_own" on public.saved_programs;
create policy "saved_programs_select_own" on public.saved_programs
  for select using (auth.uid() = user_id);

drop policy if exists "saved_programs_write_own" on public.saved_programs;
create policy "saved_programs_write_own" on public.saved_programs
  for insert with check (auth.uid() = user_id);

drop policy if exists "saved_programs_update_own" on public.saved_programs;
